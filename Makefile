...
.PHONY: help setup download schema data graph windows weights features subsample \
        ...
        ring-scorer ring-context twins anchored policy lockstep offers demo-bundle \
        label-budget propagate \
...
anchored:  ## rings extracted around anchors, with case ids that survive the night
	$(PY) -u -m orbweaver.rings.anchored
...
offers:  ## which promotions, coupon types and stimulations are being farmed
	$(PY) -u -m eval.offers

label-budget:  ## how much labelled data before this beats the base rate
	$(PY) -u -m eval.label_budget

propagate:  ## Fast Belief Propagation - spreading confirmed labels over the graph
	$(PY) -u -m orbweaver.scoring.fabp
...
reproduce: reproduce-core merchant-view replay ring-scorer ring-context twins ieee-cis anchored policy lockstep offers label-budget propagate demo-bundle  ## everything, end to end
	$(PY) -u -m eval.report
	$(PY) -u -m eval.case_report
	@echo
