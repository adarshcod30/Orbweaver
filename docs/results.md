...
**The headline, with what it costs.** Pruning to suspicious accounts first and *then* peeling for dense structure puts **0.7292** of a ring's labelled members on the fraud side, against a base rate of 0.2242 — **3.252× chance** — at a measured cost of **0.371 real customers swept into a ring for every fraudster caught**, or ₹74,400 of legitimate value at the assumed rate. Drop the score cut-off and the identical extractor falls to 0.0696 — 0.31×, worse than picking at random. [Every τ and λ tried →](#rings) · [what an hour of analyst time is worth →](#what-to-do-with-the-queue-given-a-budget)

**The finding I would defend hardest.** Dense is not the same as fraudulent, and that is not a fact about this dataset. Run unchanged on two unrelated fraud graphs, the unpruned extractor lands at 0.573× base rate on Amazon reviewers and at *exactly zero* on YelpChi — 25 rings, 1,914 accounts, not one of them fraudulent. Prune first and the same code reaches 14.3× and 6.9×. Three platforms, two of them nothing like promotion abuse, saying the same thing. [Both datasets, run unchanged →](#does-any-of-this-work-on-a-graph-that-is-not-ppa)

**13 investigations, 4 of which did not work.** Those 4 are written up beside the 9 that held, in the same table and at the same length:

- One night of data is worth nothing, and rings do not survive the night
- The crudest baseline beat the model I built to replace it
- Ring history does not transfer to the next window
- Telling a crowd from a ring by when it formed did not pay off, and the fair test explains why

[The findings table →](../README.md#the-thirteen-investigations--including-the-four-that-did-not-work) · [everything that broke on the way →](../FAILURES.md)
