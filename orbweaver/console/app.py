...
NAV_ITEMS = [("queue", "/", "Queue"), ("offers", "/offers", "Offers"),
             ("replay", "/replay", "Nightly replay"),
             ("findings", "/findings", "Findings")]

DOCS_URL = "https://adarshcod30.github.io/Orbweaver/"
REPO_URL = "https://github.com/adarshcod30/Orbweaver"

# Kept as a plain link row for the few places that want it inline in prose.
NAV = ('<p class="sub">' + " · ".join(
    f'<a href="{href}">{label.lower()}</a>' for _, href, label in NAV_ITEMS)
    + ' · <a href="/health">health</a></p>')


def masthead(active: str = "") -> str:
    """The console's own tab bar. One tab per page, the current one marked -
    a queue you can lose your place in is a queue nobody trusts."""
    tabs = "".join(
        f'<a class="tab{" on" if key == active else ""}" href="{href}"'
        + (' aria-current="page"' if key == active else '')
        + f'>{esc(label)}</a>'
        for key, href, label in NAV_ITEMS)
    return f"""<header class="masthead"><div class="masthead-in">
<a class="brand" href="/"><span class="dot"></span>Orbweaver</a>
<nav class="tabs" aria-label="Console sections">{tabs}</nav>
</div></header>"""
...
def page(title: str, description: str, body: str, active: str = "") -> str:
    ...
    the shared stylesheet, and the tab bar. Fragments HTMX swaps into #detail
    are not full documents and do not go through this."""
...
{masthead(active)}
<div class="wrap">{body}
<p class="footer">Generated by <code>make reproduce</code> — no number on this
console is typed by hand. <a href="{DOCS_URL}">Documentation and evidence</a> ·
<a href="{REPO_URL}">Source</a> · <a href="/health">Health</a></p>
</div></body></html>"""
...
<div id="detail"></div>"""
    return page("Orbweaver — offers",
                ...
                "label used in the ranking.", body, active="offers")
...
def check_card(account: int) -> str:
    ...
    return page(f"Orbweaver — account {account}",
                ...
                f'<p class="sub">answered in {took:.1f} ms</p>')
...
{figs}"""
    return page("Orbweaver — findings",
                ...
                "four of which came back negative.", body, active="findings")
